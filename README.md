...
>`Caffeine` ：一级缓存，也就是本地缓存，而内存天然就支撑高并发。
>
>`Redis` ： 二级缓存，也就是分布式缓存。


> 说明：`l2cache` 在满足高并发的同时也引入了一些新的问题，比如怎么保证分布式场景下各个节点中本地缓存的一致性问题，
> 本组件采用`数据变更通知`和`定期刷新过期缓存` 来尽量保证缓存一致性。具体见下文中的 `分布式缓存同步` 和 `缓存一致性是怎么保证的` 两个章节。
